/**
 * Фид цены ETH через Polymarket RTDS (Realtime Data Stream) — это
...
class EthPriceFeed {
  ...
  private connect(): void {
    ...
    this.ws.on("open", () => {
      console.log("[ethPriceFeed] подключено к Polymarket RTDS (Chainlink TWAP 30s)");
    ...
    this.ws.on("close", () => {
      if (this.stopped) return;
      console.log(`[ethPriceFeed] соединение закрыто, переподключение через ${RECONNECT_MS}мс`);
      this.stopPing();
      setTimeout(() => this.connect(), RECONNECT_MS);
    });

    this.ws.on("error", (err) => {
      console.error("[ethPriceFeed] ошибка:", err.message);
    });
  }
  ...
  private startWatchdog(): void {
    this.watchdogTimer = setInterval(() => {
      ...
      if (idleMs > STALE_TIMEOUT_MS) {
        console.log(`[ethPriceFeed] ⚠️ Нет TWAP-апдейтов ${Math.round(idleMs / 1000)}с — форсируем переподключение.`);
        this.forceReconnect();
      }
    }, WATCHDOG_CHECK_MS);
  }
  ...
  private forceReconnect(): void {
    ...
    try {
      ...
      console.error("[ethPriceFeed] ошибка при terminate:", (err as Error).message);
    }
    this.connect();
  }

  /** Цена ETH (TWAP), ближайшая по времени к заданному таймстемпу. null — если данных ещё нет. */
...
export const ethPriceFeed = new EthPriceFeed();
