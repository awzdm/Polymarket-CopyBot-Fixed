/**
 * Фид цены XRP через Polymarket RTDS (Realtime Data Stream) — это
...
class XrpPriceFeed {
  ...
  private startWatchdog(): void {
    this.watchdogTimer = setInterval(() => {
      ...
      if (idleMs > STALE_TIMEOUT_MS) {
        console.log(`[xrpPriceFeed] ⚠️ Нет TWAP-апдейтов ${Math.round(idleMs / 1000)}с — форсируем переподключение.`);
        this.forceReconnect();
      }
    }, WATCHDOG_CHECK_MS);
  }
  ...
  private forceReconnect(): void {
    ...
    try {
      ...
      console.error("[xrpPriceFeed] ошибка при terminate:", (err as Error).message);
    }
    this.connect();
  }

  /** Цена XRP (TWAP), ближайшая по времени к заданному таймстемпу. null — если данных ещё нет. */
