/**
 * Фид цены DOGE через Polymarket RTDS (Realtime Data Stream) — это
...
class DogePriceFeed {
  ...
  private startWatchdog(): void {
    this.watchdogTimer = setInterval(() => {
      ...
      if (idleMs > STALE_TIMEOUT_MS) {
        console.log(`[dogePriceFeed] ⚠️ Нет TWAP-апдейтов ${Math.round(idleMs / 1000)}с — форсируем переподключение.`);
        this.forceReconnect();
      }
    }, WATCHDOG_CHECK_MS);
  }
  ...
  private forceReconnect(): void {
    ...
    try {
      ...
      console.error("[dogePriceFeed] ошибка при terminate:", (err as Error).message);
    }
    this.connect();
  }

  /** Цена DOGE (TWAP), ближайшая по времени к заданному таймстемпу. null — если данных ещё нет. */
...
export const dogePriceFeed = new DogePriceFeed();
